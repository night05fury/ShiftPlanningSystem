...
// //   DELETE: after use
// //   console.log(user); 
// //   const storeusernameasID=user.username;
...
//     try {
//       const response = await axios.post("/api/employee/availability", availability);
...
//               <tr key={index}>
//                 <td className="border px-4 py-2">{moment(avail.date).format("dddd")}</td>
...
const EmployeeAvailability = () => {
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      try {
        const token=localStorage.getItem("token");
        const response = await axios.get('/api/employee/myavailability',{headers: {
